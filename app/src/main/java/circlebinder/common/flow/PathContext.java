...
import circlebinder.common.flow.util.Preconditions;
...
public class PathContext extends ContextWrapper {
    ...
    PathContext(Context leafContext, List<Screen> path, Map<String, Context> contexts) {
        super(leafContext);
        Preconditions.checkArgument(leafContext != null, "Leaf context may not be null.");
        Preconditions.checkArgument(path.size() == contexts.size(), "Path and context map are not the same size");
        if (!path.isEmpty()) {
            Screen leafScreen = path.get(path.size() - 1);
            Preconditions.checkArgument(leafContext == contexts.get(leafScreen.getName()),
